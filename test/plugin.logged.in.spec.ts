...
test.describe('Frontend – Widget', () => {
  ...
  Object.entries(heikoAdminTestData).forEach(
    ([
      ...
      test("Language '" + lang + "'", async ({ editor, page, admin }) => {
        const postId = await createPostWithPlugin(editor, admin, "Language " + lang, { userId: 1, language: lang });
      ...
      test("Language '" + lang + "', user 'heikoAdmin'", async ({ editor, page, admin }) => {
        const postId = await createPostWithPlugin(editor, admin, "User 'heikoAdmin' and language " + lang, { userId: 1, language: lang });
        await checkQuote(page, postId, lang, quotation, quotationLink, source, sourceLink, author, authorLink);
      });
    }
  );
  ...
  test("Language 'all'", async ({ editor, page, admin }) => {
    const postId = await createPostWithPlugin(editor, admin, "Language 'all'", { language: 'all' });
    await checkQuote(page, postId);
  });

  test("Language 'frontend'", async ({ editor, page, admin }) => {
    const postId = await createPostWithPlugin(editor, admin, "Language 'frontend'", { language: 'frontend' });
    await checkQuote(page, postId);
  });

  test("Language 'fr' (not supported)", async ({ editor, page, admin }) => {
  ...
  test("Language 'de', Category 'Ameise'", async ({ editor, page, admin }) => {
    const postId = await createPostWithPlugin(editor, admin, "Category 'Ameise'", { language: 'de', categoryId: 305 });
    await checkQuote(page, postId, 'de', 'Treffen sich zwei Ameisen. Fragt die eine: „Und – was machen Sie so?” Sagt die andere: „Sie meinen beruflich?”');
  });

  // author Шевченко, Тарас Григорович (id 599) and category Writing (id 292) has one quote in ukrainian
  test("Language 'uk', Author 'Шевченко', category 'Писання'", async ({ editor, page, admin }) => {
    const postId = await createPostWithPlugin(editor, admin, "Author 'Шевченко', category 'Писання'", { language: 'uk', authorId: 599, categoryId: 292});
    await checkQuote(page, postId, 'uk', null, null, null, null, heikoAdminTestData['uk']['author'], heikoAdminTestData['uk']['authorLink']);
  });
