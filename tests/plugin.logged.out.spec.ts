...
test.describe('Backend – Plugin descriptions', () => {
  ...
  test('en - English language', async ({ page }) => {
    ...
    const desc = await page.textContent('tr[data-slug="zitat-service-random-quote"] .plugin-description p') ?? "";
  ...
  test('de - German language', async ({ page }) => {
    ...
    await page.goto('/wp-admin/plugins.php');
    const desc = await page.textContent('tr[data-slug="zitat-service-zufalls-zitat"] .plugin-description p') ?? "";
    expect(desc.startsWith('Anzeige eines zufälligen Zitats')).toBeTruthy();
  });

  test('es - Spanish language', async ({ page }) => {
    await userLogin(page, ADMIN_USER + '_es', ADMIN_PASSWORD, null);
    await page.goto('/wp-admin/plugins.php');
    const desc = await page.textContent('tr[data-slug="zitat-service-cita-aleatoria"] .plugin-description p') ?? "";
